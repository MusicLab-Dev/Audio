...
#define GENERATOR_REGISTER_WAVEFORM(Name) \
    ...
    float Generate##Name(Type *output, const std::size_t outputSize, \
            const float frequencyNorm, const float phaseOffset = 0.0f, const DB gain = 1.0f) noexcept \
...
namespace Audio::DSP::Generator
{
    ...
    namespace Internal
    {
        ...
        inline float GenerateImpl(Type *output, const std::size_t outputSize,
                const float frequencyNorm, const float phaseOffset, const DB gain = 1.0f) noexcept;
        template<bool Accumulate, auto ComputeFunc, typename Type>
        ...
        template<typename Type>
        [[nodiscard]] Type GenerateSineSample(const float indexNorm, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(std::sin(indexNorm) * gain); }
        template<typename Type>
        [[nodiscard]] Type GenerateCosineSample(const float indexNorm, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(std::cos(indexNorm) * gain); }
        template<typename Type>
        [[nodiscard]] Type GenerateSquareSample(const float indexNorm, const DB gain = 1.0f) noexcept
            { return GenerateSineSample<Type>(indexNorm, gain) > 0 ? gain : -gain; }
        template<typename Type>
        [[nodiscard]] Type GenerateTriangleSample(const float indexNorm, const DB gain = 1.0f) noexcept
            { return static_cast<Type>((1.0f - std::acos(std::sin(indexNorm)) * static_cast<float>(M_2_PI)) * gain); }
        template<typename Type>
        [[nodiscard]] Type GenerateSawSample(const float indexNorm, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(-std::atan(Utils::Cot(indexNorm)) * static_cast<float>(M_2_PI) * gain); }
        template<typename Type>
        [[nodiscard]] Type GenerateNoiseSample(const float, const DB gain = 1.0f) noexcept;


        /** @brief Waveform sample generation with input as frequency modulation */
        template<typename Type>
        [[nodiscard]] Type ModulateSineSample(const float indexNorm, const float modulation, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(std::sin(indexNorm + modulation) * gain); }
        template<typename Type>
        [[nodiscard]] Type ModulateCosineSample(const float indexNorm, const float modulation, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(std::cos(indexNorm + modulation) * gain); }
        template<typename Type>
        [[nodiscard]] Type ModulateSquareSample(const float indexNorm, const float modulation, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(std::sin(indexNorm + modulation) * gain); }
        template<typename Type>
        [[nodiscard]] Type ModulateTriangleSample(const float indexNorm, const float modulation, const DB gain = 1.0f) noexcept
            { return static_cast<Type>((1.0f - std::acos(std::sin(indexNorm + modulation)) * static_cast<float>(M_2_PI)) * gain); }
        template<typename Type>
        [[nodiscard]] Type ModulateSawSample(const float indexNorm, const float modulation, const DB gain = 1.0f) noexcept
            { return static_cast<Type>(-std::atan(Utils::Cot(indexNorm + modulation)) * static_cast<float>(M_2_PI) * gain); }
        template<typename Type>
        [[nodiscard]] Type ModulateNoiseSample(const float indexNorm, const float, const DB gain = 1.0f) noexcept
            { return GenerateNoiseSample<Type>(indexNorm, gain); }
