...
public:
    ...
    [[nodiscard]] static std::size_t GetResampleSemitoneBufferSize(const std::size_t inputSize, const SampleRate inSampleRate, const bool upScale) noexcept;
    ...
    [[nodiscard]] static std::size_t GetResampleOctaveBufferSize(const std::size_t inputSize, const SampleRate sampleRate, const int nOctave) noexcept;
    ...
    void resampleSemitone(const Type *inputBuffer, Type *outputBuffer, const std::size_t inputSize, const SampleRate inSampleRate, const bool upScale);

    /** @brief Resample the inputBuffer into outputBuffer, shifting by an octave count */
    template<bool Accumulate, unsigned ProcessSize>
    void resampleOctave(const Type *inputBuffer, Type *outputBuffer, const std::size_t inputSize, const SampleRate sampleRate, const int nOctave, const std::size_t inputOffset = 0u);
    ...
    void resampleSampleRate(const Type *inputBuffer, Type *outputBuffer, const std::size_t inputSize, const SampleRate inSampleRate, const SampleRate outSampleRate);
