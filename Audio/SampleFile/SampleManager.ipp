...
template<typename T>
inline Audio::Buffer Audio::SampleManager<T>::LoadSampleFile(const std::string &path, SampleSpecs &specs)
{
    ...
    return LoadSampleFileExtension(path, ext.string(), specs);
}

template<typename T>
inline Audio::Buffer Audio::SampleManager<T>::LoadSampleFileExtension(const std::string &path, const std::string &ext, SampleSpecs &specs)
{
    for (auto i = 0; i < sizeof(SupportedExtension) / sizeof(std::pair<int, int>); ++i) {
        if (std::string(std::get<0>(SupportedExtension[i])) == ext)
            return std::get<1>(SupportedExtension[i])(path, specs);
    }
